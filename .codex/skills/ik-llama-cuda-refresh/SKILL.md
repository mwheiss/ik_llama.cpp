...
description: Refresh /storage/ik_llama.cpp from canonical upstream, preserve its request-scoped CUDA graph cache cleanup and cleanly replayable runtime RoPE frequency-scale override, rebuild the existing CUDA configuration, and synchronize the rebased result to the mwheiss fork when authorized. Use for recurring ik_llama.cpp update, local patch replay, fork synchronization, or normal local rebuild requests.
---
...
- Keep the CUDA cleanup as one commit, the runtime RoPE frequency-scale change
  as one commit, and this project skill as a separate commit on top of
  `upstream/main`, in that order. Rebase them when the worktree is clean. If
  conflict resolution changes the cleanup or skill commit, inspect the final
  diff and retain coherent atomic commits. Apply the stricter fail-closed rule
  below to the RoPE commit.
...
## Replay the runtime RoPE frequency scale

Retain the runtime RoPE frequency-scale commit only while it rebases as a
clean, non-empty commit and upstream does not already provide the same behavior.
Inspect `llm_build_context::build_std_attention` rather than relying on commit
subjects. Preserve this exact distinction:

- standard non-SWA attention uses `cparams.rope_freq_scale`, honoring the
  runtime `llama_context_params.rope_freq_scale` value; and
- SWA attention continues to use `hparams.rope_freq_scale_train_swa`.

Verify that the runtime parameter still flows into `cparams.rope_freq_scale`.
Keep the patch scoped to the one assignment in
`src/llama-build-context.cpp`. If upstream already implements the behavior,
omit the local commit and cite the exact implementation. If the commit
conflicts, becomes empty for an unclear reason, or would require adapting to
changed APIs, stop and report the conflict; do not resolve or broaden it
without fresh user direction.
